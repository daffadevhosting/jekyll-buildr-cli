...
The CLI version [![NPM Version](https://img.shields.io/npm/v/jekyll-buildr-cli.svg)](https://www.npmjs.com/package/jekyll-buildr-cli) is compatible with the Jekyll Buildr web application and backend API. The CLI communicates with the webapp's API endpoints at `https://jekyll-buildr.vercel.app/api` to provide AI-powered features. The CLI and webapp are designed to work together seamlessly, with the webapp handling authentication and AI processing while the CLI provides a convenient terminal interface.
